            ...
            <Form className='grid gap-2' method='post' action='/transactions'>
                ...
                {categories.length ? (
                    ...
                    <select className='input border-slate-700' name='category' required>
                        {categories.map((ctg, idx) => (
                            <option className='bg-slate-700' key={idx} value={ctg.id}>{ctg.title}</option>
                        ))}
                    </select>
                </label>
                ) : (
                ...
                <div className="flex gap-4 items-center">
                    <label className='flex cursor-pointer items-center gap-2'>
                        <input name='type' type='radio' value={'income'} className='form-radio text-blue-600'/>
                        <span>Income</span>
                    </label>
                    <label className='flex cursor-pointer items-center gap-2'>
                        <input name='type' type='radio' value={'expense'} className='form-radio text-blue-600'/>
                        <span>Expense</span>
                    </label>
                </div>
